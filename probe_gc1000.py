...
def read_assembly(drv, class_code, instance, attribute):
    """One get_attribute_single. Returns (bytes_or_None, error_str_or_None)."""
    try:
        r = drv.generic_message(
...
def do_scan(drv, args) -> None:
    ...
    for inst in parse_instances(args.scan):
        data, err = read_assembly(drv, args.class_code, inst, args.attribute)
        if data is not None:
            hits += 1
...
def do_watch(drv, args) -> None:
    ...
    while True:
        data, err = read_assembly(drv, args.class_code, args.instance, args.attribute)
        if data is None:
